...
export class SuiPythClient {
  ...
  constructor(
    ...
    priceFeedObjectIdCache?: Record<string, ObjectId>
  ) {
   ...
   */
  async verifyVaas(wormholePackageId: string, vaas: Buffer[], tx: Transaction) {
    const verifiedVaas: any[] = [];
  ...
  async updatePriceFeeds(
    ...
    const [packageId, baseUpdateFee, wormholePackageId] = await Promise.all([
    ...
    const verifiedVaas = await this.verifyVaas(wormholePackageId, [vaa], tx);
